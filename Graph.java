...
public class Graph extends JPanel implements MouseListener {
                    ...
                    if (input == null) {
                        ...
                        if (input.length() == 1 && validVertexID(input)) {
                            ...
                            JOptionPane.showMessageDialog(this,
                                    "The Vertex ID must be exactly 1 character.",
                                    "Invalid Vertex ID", JOptionPane.ERROR_MESSAGE);
                        } else {
                            JOptionPane.showMessageDialog(this,
                                    "A vertex with ID \"" + input + "\" already exists.",
                                    "Duplicate Vertex ID", JOptionPane.ERROR_MESSAGE);
                        }
                    }
                }
            }
                ...
                if (edgeVertices.size() == 2) {
                    ...
                    for (List<String> verticesOfAnEdge: availableEdges) {
                        ...
                        if (verticesOfAnEdge.contains(id1) && verticesOfAnEdge.contains(id2)) {
                            JOptionPane.showMessageDialog(this,
                                    "An edge between " + id1 + " and " + id2 + " already exists.",
                                    "Invalid Edge", JOptionPane.ERROR_MESSAGE);
                            edgeVertices.clear();
                            return;
                        }
    ...
    private void drawEdge(Vertex vertex1, Vertex vertex2) {
        while (true) {
            String input = JOptionPane.showInputDialog(this, "Enter Weight (non-negative whole number):",
                    "Input", JOptionPane.QUESTION_MESSAGE);
            if (input == null) {
                break;
            } else {
                input = input.trim();
                // Negative weights are rejected: Dijkstra's and Prim's/Kruskal's
                // MST algorithms are only guaranteed correct on non-negative weights.
